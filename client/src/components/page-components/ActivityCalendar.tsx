...
const ActivityCalendar: React.FC<ActivityCalendarProps> = ({
  ...
  const getDateForWeekAndDay = (weekOffset: number, dayIndex: number) => {
  ...
  const getWeekIndexForDate = (date: Date) => {
    // Match the backend calculation: Math.ceil((match.date + 86400 * 2) / 604800)
  ...
  return (
    <div className="w-full mt-4">
      <div className="bg-warmscale-8 rounded-md px-4 py-3 drop-shadow-sm font-cantarell">
              ...
              <div key={weekOffset} className="flex flex-col gap-1 flex-1">
                {daysOfTheWeek.map((day, dayIndex) => {
                  ...
                  const games = weekData
                    ? weekData[day as keyof typeof weekData]
                    : 0;
