...
const Logs = () => {
  ...
  const handleClickOutside = (e: any) => {
    if (
      tab == 'scoreboard' &&
      refOne.current.contains(e.target) !== null &&
    ...
    if (
      tab == 'performance' &&
      refTwo.current.contains(e.target) !== null &&
  ...
  function ID3toID64Converter(userId3: string) {
    const steamid64ent1 = 7656,
      steamid64ent2 = 1197960265728,
      cleanId3 = userId3.replace(/\]/g, '').split(':')[2];
    const userid64 = steamid64ent1 + String(parseInt(cleanId3) + steamid64ent2);
    return userid64;
  }

  function scoreboardSorter(sortBy: string) {
    const sortedArray: any = [];
    const unsortedArray: any = scoreboard;
  ...
  function ptscoreboardSorter(sortBy: string) {
    const sortedArray: any = [];
    const unsortedArray: any = ptscoreboard;
  ...
  async function demostfLinkIdFinder() {
    const currentPlayersArray: any = Object.entries(apiResponse.players);
    let currentResponse: any = '';
    for (let index = 0; index < 1; index++) {
      currentResponse += currentPlayersArray[index][0];
    }
    const demostfApiResponse: any = await fetch(
      `https://api.demos.tf/demos/?map=${apiResponse.info.map}&players[]=${currentResponse}&after=${apiResponse.info.date - 5000}&before=${apiResponse.info.date + 5000}`,
      FetchResultTypes.JSON
    );
  ...
  function killSpreadSorter(sortBy: string) {
    ...
    const unsortedArray = killSpread;
    const sortedArray = [];
    if (sortBy !== killSpreadSort && sortBy !== 'kills') {
      for (
        ...
        for (
          ...
          const currentPlayerArray: any = Object.entries(
            ...
            >
              <div id="match-info" className="flex gap-7">
                ...
                <div className="block">
                  ...
                  >
                    {apiResponse.info.map.split('_')[1] !== undefined
                      ? apiResponse.info.map
                          .split('_')[1]
                          .charAt(0)
                          .toUpperCase() +
                        apiResponse.info.map.split('_')[1].slice(1)
                      : apiResponse.info.map}
                  </div>
                </div>
              ...
              >
                <a
                  target="_blank"
                  href={`https://demos.tf/${currentDemosID}`}
            ...
            >
              <div id="scoreboard" className="p-2 rounded-md m-4">
                <div className=" h-1 flex bg-warmscale-4 mt-8 mb-10 mx-2 justify-between">
                  {apiResponse.rounds.map((round: any, index: any) => {
                    return (
                      <div
                        key={index}
                        className={` ${
                          apiResponse.info.map.includes('pl_')
                            ? index === 1 || index % 2 === 1
                              ? 'bg-tf-blue-dark'
                              : 'bg-tf-red-dark'
                        ...
                        style={{
                          width: `${
                            (round.roundDuration +
                              15 / apiResponse.info.matchLength) *
                            100
                          }%`,
                        }}
                      >
                      ...
                      scoreboard.map((player: any, index: any) => {
                        ...
                        function stat(
                          ...
                          return (
                            ...
                            >
                              {playerObject[statInput] === null
                                ? 0
                                : playerObject[statInput].toFixed(
                                    decimalPlaces
                                  )}
                            </div>
                          );
              ...
              {apiResponse.info.passTime === 1 && (
                <div className="w-full flex justify-center items-center">
                  <div className="bg-warmscale-82 p-3 w-fit rounded-md">
                    <div className="text-lg text-lightscale-3 font-semibold font-cantarell text-center mb-3">
                      PASS TIME
                    </div>
                    <div id="stat-titles">
                      <div
                      ...
                      >
                        ...
                        {ptstatTitle(
                          'failedPass',
                          'FP',
                          'Failed Passes - Interceptions that occur within 1 second'
                        )}
                    ...
                    {apiResponse !== undefined &&
                      ptscoreboard.map((player: any, index: any) => {
                        ...
                        if (playerObject.damageTaken !== 0) {
                          return (
                            <div
                              key={`plscoreboard-${player[0]}-${index}`}
                              id="player-stat-card"
                            >
                              <div
                                className={`grid h-10 border-b border-warmscale-8 ${
                            ...
                            >
                              {statInput === 'scores' && (
                                <div className="absolute group-hover:scale-100 scale-0 bg-warmscale-85 rounded-sm drop-shadow border-warmscale-4 border z-50 p-2 bottom-9">
                                  <div className="text-xs font-semibold font-cantarell text-lightscale-6 text-center  border-b border-b-warmscale-6 mb-1">
                                    PANACEAS
                                  </div>
                                  <div className="text-center font-semibold text-xs ">
                                    {player[1].passTime.panaceas === 0
                                      ? 0
                                      : player[1].passTime.panaceas.toFixed(
                                          decimalPlaces
                                        )}
                                  </div>
                              ...
                              {player[1].passTime[statInput] === 0
                                ? 0
                                : player[1].passTime[statInput].toFixed(
                                    decimalPlaces
                                  )}
                            </div>
                          );
                    ...
                    return (
                      ...
                      >
                        ...
                        {classOrder.map((currentClass, index) => {
                          const currentArray: any = Object.entries(killer[1]);
              ...
              <div className="flex h-full">
                {currentPerformanceFocus !== '' && (
                  <div className="flex ml-10 mr-10 mb-10">
                          ...
                          <div id="damage-division-chart">
                            {performanceChartSort === 'dealt' &&
                              Object.entries(
                                ...
                                const objectA: any = Object.entries(
                                  apiResponse.players[currentPerformanceFocus]
                                    .damageDivision.damageTo
                                )[0][1];
                                const objectB: any = Object.entries(
                                  apiResponse.players[currentPerformanceFocus]
                                    .damageDivision.damageFrom
                                )[0][1];
                                const currentMax: any = Math.max(
                                  objectA,
                                  objectB
                                );
                            ...
                            {performanceChartSort === 'class' &&
                              damageDivisionSortByClass().map(
                                (player: any, index: any) => {
                                  const currentPlayerArray: Array<any> =
                    ...
                    {apiResponse.info.map.split('_')[1] !== undefined && (
                      <div
                        id="performance-killmap-container"
                      ...
                      >
                        ...
                        <div
                          onClick={() => {
                            if (killMapActive === false) setKillMapActive(true);
                          }}
                          ref={refTwo}
                        ...
                        >
                          <div className="bg-black items-center relative flex">
                            <div
                              onClick={() => {
                                if (killMapActive) setKillMapActive(false);
                              }}
                            ...
                            <div className="h-full aspect-square absolute">
                              <div className="absolute aspect-square h-full">
                                ...
                                >
                                  {apiResponse.events.map(
                                    (killEvent: any, index: any) => {
                                      const mapName =
                                      ...
                                      const currentScale = currentMap.scale;
                                      const calibrationX = currentMap.x;
                                      const calibrationY = currentMap.y;
                                      if (
                                        killEvent[
                                          killMapShowDeaths
                                            ? 'victimId'
                                        ...
                                        return (
                                          ...
                                          >
                                            <line
                                              x1={`${((killEvent.killer.position.x - (calibrationX + 910 * currentScale)) / currentScale) * 0.097656 + 50}%`}
                                              y1={`${((killEvent.killer.position.y - (calibrationY - 512 * currentScale)) / currentScale) * -0.097656 + 50}%`}
                                              x2={`${((killEvent.victim.position.x - (calibrationX + 910 * currentScale)) / currentScale) * 0.097656 + 50}%`}
                                              y2={`${((killEvent.victim.position.y - (calibrationY - 512 * currentScale)) / currentScale) * -0.097656 + 50}%`}
                    ...
                    {apiResponse.info.map.split('_')[1] === undefined && (
                      <div className=" text-warmscale-2 font-semibold flex justify-center items-center ml-4 text-xl font-robotomono">
                        THE MAP IS BEING REWORKED FOR COMBINED LOGS
                      </div>
                    )}
                  </div>
                )}
            ...
            >
              <div className="flex justify-center gap-10 mx-10 my-4">
                <div className="bg-warmscale-82 rounded-md pb-2 px-4 w-96 h-fit">
                  ...
                  {apiResponse.chat.map((chatEvent: any, index: number) => {
                    return (
                      <div
                        key={`chat-${chatEvent.userId}-${index}`}
                        className="py-0.5 text-lightscale-2"
                      >
                        {' '}
                        <span
                          className={`${
                            apiResponse.players[chatEvent.userId] !==
                              undefined &&
                        ...
                        >
                          {apiResponse.players[chatEvent.userId] !== undefined
                ...
                {apiResponse.killStreaks.source !== 'internal' && (
                  <div className="bg-warmscale-82 rounded-md pb-2 px-4 w-96 h-fit ">
                    <div className="flex justify-center text-lightscale-2 font-semibold text-xl my-2">
                      KILLSTREAKS
                    </div>
                    ...
                    {Object.entries(apiResponse.killStreaks).map(
                      (killStreakEvent: any, index: number) => {
                        return (
                          <div
                            key={`killstreak-${killStreakEvent[0]}-${index}`}
                            className="py-0.5 text-lightscale-2 flex justify-between"
                          >
                            <div
                              className={`${
                                apiResponse.players[
