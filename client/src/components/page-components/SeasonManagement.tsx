...
const SeasonManagement = () => {
  ...
  const handleUpdateSeason = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/season-management/seasons/${editingSeason.seasonid}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleToggleActive = async (seasonid: number) => {
    try {
      const response = await fetch(
        `/api/season-management/seasons/${seasonid}/toggle-active`,
        {
          method: 'PATCH',
          credentials: 'include',
        }
  ...
  const handleToggleDisplayCard = async (seasonid: number) => {
    try {
      const response = await fetch(
        `/api/season-management/seasons/${seasonid}/toggle-displaycard`,
        {
          method: 'PATCH',
          credentials: 'include',
        }
  ...
  const handleDeleteSeason = async (seasonid: number, seasonname: string) => {
    ...
    try {
      const response = await fetch(
        `/api/season-management/seasons/${seasonid}`,
        {
          method: 'DELETE',
          credentials: 'include',
        }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-warmscale-7">
      ...
      <div className="flex-grow container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-warmscale-1 mb-2">
            Season Management
          </h1>
          <p className="text-warmscale-2">
            Manage seasons for RGL and OZF leagues
          </p>
        </div>
        ...
        <div className="bg-warmscale-6 rounded-lg p-6 mb-6 border border-warmscale-5">
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[200px]">
              <label className="block text-warmscale-1 mb-2 font-medium">
                Filter by League
              </label>
              <select
                value={filterLeague}
        ...
        {(showAddForm || editingSeason) && (
          <div className="bg-warmscale-6 rounded-lg p-6 mb-6 border-2 border-accent-500">
            ...
            <form
              onSubmit={editingSeason ? handleUpdateSeason : handleAddSeason}
            >
              ...
              <div className="mb-4">
                <label className="flex items-center text-warmscale-1 cursor-pointer">
                  ...
                  <span className="ml-2 text-warmscale-3 text-sm">
                    (Only one season can be active per league/format)
                  </span>
                </label>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={
                    editingSeason ? cancelEdit : () => setShowAddForm(false)
                  }
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-warmscale-5">
                <tr>
                  <th className="px-6 py-3 text-left text-warmscale-1 font-semibold">
                    Season ID
                  </th>
                  <th className="px-6 py-3 text-left text-warmscale-1 font-semibold">
                    Season Name
              ...
              <tbody className="divide-y divide-warmscale-5">
                {filteredSeasons.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="px-6 py-8 text-center text-warmscale-3"
                    >
                      No seasons found
                    </td>
                  </tr>
                  ...
                  filteredSeasons.map((season) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() =>
                            handleToggleDisplayCard(season.seasonid)
                          }
                          className={`px-4 py-1 rounded-full text-sm font-semibold transition-colors ${
                            season.displaycard
