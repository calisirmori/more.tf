...
import { RGL_SEASONS, OZF_SEASONS, ALL_SEASONS, getSeasonsByFormat } from "../../data/seasons";
...
const UnifiedSeasonSummary = () => {
  ...
  async function getLastSummaryData() {
    let response: any;
    try {
      const endpoint = selectedLeague === "RGL"
  ...
  async function getSummaryData() {
    ...
    try {
      ...
      const uniqueDivisions = [...new Set(response.rows.map((row: any) => row.division))].filter(Boolean);
      ...
      if (uniqueDivisions.length > 0 && !uniqueDivisions.includes(currentDivision)) {
  ...
  function calculateValue(
    ...
    switch (sortKey) {
      ...
      case "specialty":
        ...
        return (
          Math.round(
            (playerStat[classSpecs[selectedFormat][currentClass].id] / playTime) * 100
          ) / 100
        );
      case "time":
  ...
  const configuredDivisions = selectedSeasonId ? getDivisionsForSeason(
    selectedSeasonId,
    selectedFormat,
    selectedLeague
  ) : [];
  ...
  const divisions = detectedDivisions.length > 0
        ...
        <div className="bg-warmscale-8 rounded-md relative w-full shadow-lg">
            ...
            <div className="px-3 py-4 md:px-8 md:py-6 border-b border-warmscale-7">
              <div className="flex flex-col md:flex-row gap-4 md:gap-6 justify-center items-stretch md:items-center">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-warmscale-3 text-xs md:text-sm font-bold">League</label>
                ...
                {/* Format Filter */}
                <div className="flex flex-col gap-2">
                  <label className="text-warmscale-3 text-xs md:text-sm font-bold">Format</label>
                  <div className="flex gap-2">
                    <button
                ...
                {/* Season Filter */}
                <div className="flex flex-col gap-2">
                  <label className="text-warmscale-3 text-xs md:text-sm font-bold">Season</label>
                  <select
            ...
            <div className="absolute right-2 top-2 md:top-1.5 z-10">
              <div className="flex justify-center gap-1">
                <div className="text-xs md:text-sm text-warmscale-3 font-bold">time filter:</div>
                <div
            ...
            <div className="md:hidden px-3 py-3 border-b border-warmscale-7">
              <div className="relative">
                ...
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                  <svg
                    className="w-4 h-4 fill-lightscale-1"
                    viewBox="0 0 20 20"
                  >
            ...
            <div className="p-2 overflow-x-auto lg:overflow-x-visible">
              ...
              {displayArray.map((currentPlayer: any, index: number) => {
                ...
                const userName = selectedLeague === "RGL" ? currentPlayer.rglname : currentPlayer.ozfname;
                const teamID =
