...
async function parse(LogFile, matchID, logsApiResponse) {
  ...
  let parsedJSON = {
    ...
    info: {
      ...
      matchLength:
        eventDateToSeconds(unparsedArray[lastIndex - 5]) -
        eventDateToSeconds(unparsedArray[0]),
      winner: 'live',
      status: true,
      pause: {
        lastPause: 0,
      ...
      map:
        logsApiResponse.info !== undefined
          ? logsApiResponse.info.map
          : logsApiResponse.map,
      logsTitle:
        logsApiResponse.info !== undefined
          ? logsApiResponse.info.title
          : logsApiResponse.title,
      passTime: 0,
    },
    teams: {
      red: {
    ...
    killStreaks:
      logsApiResponse.killstreaks !== undefined
        ? logsApiResponse.killstreaks
        : { source: 'internal' },
  };
  ...
  for (let lineIndex = 0; lineIndex < unparsedArray.length; lineIndex++) {
    ...
    unparsedEvent.includes('triggered')
      ? triggeredEvent(unparsedEvent, parsedJSON, playerIDFinder)
      : nonTriggeredEvent(
          unparsedEvent,
          parsedJSON,
          playerIDFinder,
