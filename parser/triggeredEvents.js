...
const {
  statPercentiles,
  statWeights,
  finalScorePercentiles,
} = require('./combatScoreData');
...
async function worldEvents(unparsedEvent, finalObject) {
  if (unparsedEvent.includes('Round_Start')) {
    ...
    if (
      finalObject.rounds.length === 0 ||
      finalObject.rounds[finalObject.rounds.length - 1].roundDuration !== 'live'
    )
    ...
    finalObject.rounds[finalObject.rounds.length - 1].roundWinner =
      unparsedEvent.includes('Red') ? 'red' : 'blue';
  } else if (unparsedEvent.includes('Round_Length')) {
    finalObject.rounds[finalObject.rounds.length - 1].roundDuration = Math.ceil(
      parseInt(
        unparsedEvent.slice(
          unparsedEvent.indexOf('(seconds ') + 10,
          unparsedEvent.lastIndexOf('"')
        )
      )
    );
  } else if (unparsedEvent.includes('Round_Win')) {
    ...
    finalObject.info.winner =
      finalObject.teams.red.score > finalObject.teams.blue.score
        ? 'red'
        : 'blue';
    finalObject.info.matchLength =
      eventDateToSeconds(unparsedEvent) - finalObject.info.date;
    let minutesInMatch = finalObject.info.matchLength / 60;

    // await rglData(finalObject);
    for (
      let playerIndex = 0;
      playerIndex < finalObjectArray.length &&
      finalObject.players[finalObjectArray[playerIndex][0]].class !==
        'undefined';
      playerIndex++
    ) {
      ...
      player.classStats[currentClass].time +=
        matchEndTime - player.classStats.changedClass;
      player.damagePerMinute = Math.ceil(player.damage / minutesInMatch);
      player.damageTakenPerMinute = Math.ceil(
        player.damageTaken / minutesInMatch
      );
      player.healsPerMinute = Math.ceil(player.heals / minutesInMatch);
      player.killAssistPerDeath =
        Math.round((player.kills + player.assists) / (player.deaths / 10)) / 10;
      player.killsPerDeath =
        Math.round(player.kills / (player.deaths / 10)) / 10;
      player.medicStats.uberLength =
        player.medicStats.ubers !== 0
          ? Math.round(
              player.medicStats.uberLength / (player.medicStats.ubers / 10)
            ) / 10
          : 0;
      player.damageDivision.damageTo = Object.entries(
        player.damageDivision.damageTo
      )
        .sort(([, b], [, a]) => a - b)
        .reduce((r, [k, v]) => ({ ...r, [k]: v }), {});
      player.damageDivision.damageFrom = Object.entries(
        player.damageDivision.damageFrom
      )
        .sort(([, b], [, a]) => a - b)
        .reduce((r, [k, v]) => ({ ...r, [k]: v }), {});
      if (
        finalObject.healSpread[finalObjectArray[playerIndex][0]] !== undefined
      ) {
        finalObject.healSpread[finalObjectArray[playerIndex][0]] =
          Object.entries(
            finalObject.healSpread[finalObjectArray[playerIndex][0]]
          )
            .sort(([, b], [, a]) => a - b)
      ...
      let stats =
        currentClass === 'medic'
          ? ['heals', 'apm', 'deathpm', 'dtm']
          : ['kpm', 'dpm', 'apm', 'deathpm', 'dtm'];
      let combatStats = {
      ...
      const targetedScorePercentiles =
        finalScorePercentiles[mapName][currentClass];
      for (
        let combatScoreIndex = 0;
        combatScoreIndex < targetedScorePercentiles.length;
        combatScoreIndex++
      ) {
        if (targetedScorePercentiles[combatScoreIndex] > currentScoreSum) {
          let lowerScore =
            combatScoreIndex > 0
              ? targetedScorePercentiles[combatScoreIndex - 1]
              : 0;
          ...
          let tenthsPlace =
            differenceBetweenRealScoreAndLow / differenceBetweenLowHigh;
      ...
      finalObject.players[finalObjectArray[playerIndex][0]].combatScore =
        playerCombatScore === -1 ? 100 : playerCombatScore;
    }
  }
}
...
function passTimeEvents(unparsedEvent, finalObject, playerIDFinder) {
  ...
  function incrementStat(player, stat) {
    player.passTime[stat] =
      player.passTime[stat] === undefined ? 1 : player.passTime[stat] + 1;
  }
  ...
  if (unparsedEvent.includes('"pass_pass_caught"')) {
    ...
    if (
      unparsedEvent.includes('(duration "0') &&
      unparsedEvent.includes('(interception "1")')
    ) {
...
function pointsCappedEvent(unparsedEvent, finalObject, playerIDFinder) {
  ...
  if (finalObject.rounds[finalObject.rounds.length - 1].firstCap === 'live') {
    finalObject.rounds[finalObject.rounds.length - 1].firstCap =
      unparsedEvent.slice(
        unparsedEvent.indexOf('Team "') + 6,
        unparsedEvent.indexOf('" triggered "pointcaptured')
      ) === 'Red'
        ? 'red'
        : 'blue';
    finalObject.teams[
  ...
  for (let index = 1; index < eventPlayerArray.length; index++) {
    let playerId3 = eventPlayerArray[index].slice(
      eventPlayerArray[index].indexOf('[U:1:'),
      eventPlayerArray[index].indexOf(']>') + 1
    );
  ...
  let captureEventObject = {
    team: unparsedEvent.slice(
      unparsedEvent.indexOf('Team "') + 6,
      unparsedEvent.indexOf('" triggered "pointcaptured')
    ),
    time:
      eventDateToSeconds(unparsedEvent) -
      finalObject.rounds[finalObject.rounds.length - 1].roundBegin,
    name: unparsedEvent.slice(
      unparsedEvent.indexOf('"pointcaptured" (cp "') + 21,
      unparsedEvent.indexOf('") (cpname')
    ),
  };
  finalObject.rounds[finalObject.rounds.length - 1].captureEvents.push(
    captureEventObject
  );
}
...
function medicEvents(unparsedEvent, finalObject, playerIDFinder) {
  if (unparsedEvent.includes('"medic_death_ex" (uberpct "9')) {
    let medicId3 = unparsedEvent.slice(
      unparsedEvent.indexOf('[U:1:'),
      unparsedEvent.indexOf(']>') + 1
    );
    finalObject.players[playerIDFinder[medicId3]].medicStats.nearFullDeaths++;
  } else if (unparsedEvent.includes('medic_death')) {
    let killerId3 = unparsedEvent.slice(
      unparsedEvent.indexOf('[U:1:'),
      unparsedEvent.indexOf(']>') + 1
    );
    let medicId3 = unparsedEvent.slice(
      unparsedEvent.lastIndexOf('[U:1:'),
      unparsedEvent.lastIndexOf(']>') + 1
    );
    ...
    if (unparsedEvent.includes('ubercharge "1"')) {
      ...
      finalObject.teams[finalObject.players[playerIDFinder[killerId3]].team]
        .drops++;
    }
  }
}
...
function uberChargeEvents(unparsedEvent, finalObject, playerIDFinder) {
  ...
  if (unparsedEvent.includes(' "chargedeployed" ')) {
    let medigunType = unparsedEvent.slice(
      unparsedEvent.indexOf('" (medigun "') + 12,
      unparsedEvent.lastIndexOf('")')
    );
    finalObject.players[playerIDFinder[medicId3]].medicStats.ubers++;
    finalObject.teams[finalObject.players[playerIDFinder[medicId3]].team]
      .charges++;
    if (
      finalObject.players[playerIDFinder[medicId3]].medicStats.uberTypes[
        medigunType
      ] === undefined
    ) {
      finalObject.players[playerIDFinder[medicId3]].medicStats.uberTypes[
    ...
    let uberLength = parseFloat(
      unparsedEvent.slice(
        unparsedEvent.indexOf('" (duration "') + 13,
        unparsedEvent.lastIndexOf('")')
      )
    );
    finalObject.players[playerIDFinder[medicId3]].medicStats.uberLength +=
      uberLength;
  } else if (unparsedEvent.includes('"first_heal_after_spawn"')) {
    let healAfterSpawnTime = parseFloat(
      unparsedEvent.slice(
        unparsedEvent.indexOf('(time ') + 7,
        unparsedEvent.lastIndexOf('")')
      )
    );
    finalObject.players[playerIDFinder[medicId3]].medicStats.healAfterSpawn +=
      healAfterSpawnTime;
  }
}

function commonEvents(unparsedEvent, finalObject, playerIDFinder, eventType) {
  let userId3 = unparsedEvent.slice(
    unparsedEvent.indexOf('[U:1:'),
    unparsedEvent.indexOf(']>') + 1
  );
...
function healEvent(unparsedEvent, finalObject, playerIDFinder) {
  let healerId3 = unparsedEvent.slice(
    unparsedEvent.indexOf('[U:1:'),
    unparsedEvent.indexOf(']>') + 1
  );
  if (healerId3 === '') {
    let currentId1 = unparsedEvent.slice(
      unparsedEvent.indexOf('<STEAM_') + 7,
      unparsedEvent.lastIndexOf('><')
    );
  ...
  let recieverId3 = unparsedEvent.slice(
    unparsedEvent.lastIndexOf('[U:1:'),
    unparsedEvent.lastIndexOf(']>') + 1
  );
  if (recieverId3 === '') {
  ...
  let healDone = parseInt(
    unparsedEvent.slice(
      unparsedEvent.indexOf('(healing "') + 10,
      unparsedEvent.lastIndexOf('")')
    )
  );

  healDone > 80 &&
    (finalObject.players[playerIDFinder[healerId3]].crossbowHealing +=
      healDone);
  ...
  // heals per interval
  let timeInterval = 30;
  let currentIntervalIndex = Math.floor(
    (eventDateToSeconds(unparsedEvent) - finalObject.info.date) / timeInterval
  );
  if (
    finalObject.healsPerInterval[
      finalObject.players[playerIDFinder[healerId3]].team
    ...
    finalObject.healsPerInterval[
      finalObject.players[playerIDFinder[healerId3]].team
    ][currentIntervalIndex] += healDone;
  }
}

function assistEvent(unparsedEvent, finalObject, playerIDFinder) {
  let assisterId3 = unparsedEvent.slice(
    unparsedEvent.indexOf('[U:1:'),
    unparsedEvent.indexOf(']>') + 1
  );
  let victimId3 = unparsedEvent.slice(
    unparsedEvent.lastIndexOf('[U:1:'),
    unparsedEvent.lastIndexOf(']>') + 1
  );
  ...
  let currentAssisterClass =
    finalObject.players[playerIDFinder[assisterId3]].class;
  finalObject.players[playerIDFinder[assisterId3]].classStats[
    currentAssisterClass
  ].assists++;
...
function pauseEvents(unparsedEvent, finalObject) {
  if (unparsedEvent.includes('unpause')) {
    finalObject.info.pause.pauseSum =
      eventDateToSeconds(unparsedEvent) - finalObject.info.pause.lastPuase;
  } else {
...
function shotEvents(unparsedEvent, finalObject, playerIDFinder) {
  ...
  let weaponUsed = unparsedEvent.slice(
    unparsedEvent.indexOf('" (weapon "') + 11,
    unparsedEvent.lastIndexOf('")')
  );
  ...
  if (
    ...
    finalObject.players[playerIDFinder[userId3]].classStats[currentClass]
      .weapons[weaponUsed][
      unparsedEvent.includes('shot_hit') ? 'shotsHit' : 'shotsFired'
    ]++;
  } else {
    finalObject.players[playerIDFinder[userId3]].classStats[
      currentClass
    ].weapons[weaponUsed] = {
      kills: 0,
      damage: 0,
...
function damageEvent(unparsedEvent, finalObject, playerIDFinder) {
  let damageDealerId3 = unparsedEvent.slice(
    unparsedEvent.indexOf('[U:1:'),
    unparsedEvent.indexOf(']>') + 1
  );
  if (damageDealerId3 === '') {
  ...
  let damageRecieverId3 = unparsedEvent.slice(
    unparsedEvent.lastIndexOf('[U:1:'),
    unparsedEvent.lastIndexOf(']>') + 1
  );
  ...
  let damageDealt = parseInt(
    unparsedEvent.slice(
      unparsedEvent.indexOf('(damage "') + 9,
      unparsedEvent.lastIndexOf('") (weapon')
    )
  );

  // Check if both players exist before accessing
  if (
    !finalObject.players[playerIDFinder[damageDealerId3]] ||
    !finalObject.players[playerIDFinder[damageRecieverId3]]
  ) {
    return;
  }

  damageDealt === 0 &&
    finalObject.players[playerIDFinder[damageDealerId3]].uberHits++;

  //weapon finder
  let weaponUsed;
  if (unparsedEvent.includes('healing')) {
    weaponUsed = unparsedEvent.slice(
      unparsedEvent.indexOf('(weapon') + 9,
      unparsedEvent.lastIndexOf('") (healing')
    );
  } else if (unparsedEvent.includes('crit')) {
    weaponUsed = unparsedEvent.slice(
      unparsedEvent.indexOf('(weapon') + 9,
      unparsedEvent.indexOf('") (crit')
    );
    ...
    weaponUsed = unparsedEvent.slice(
      unparsedEvent.indexOf('(weapon') + 9,
      unparsedEvent.lastIndexOf('") (airshot')
    );
  ...
  if (unparsedEvent.includes('realdamage')) {
    damageDealt = parseInt(
      unparsedEvent.slice(
        unparsedEvent.indexOf('(damage "') + 9,
        unparsedEvent.lastIndexOf('") (realdamage')
      )
    );
    currentReal = parseInt(
      unparsedEvent.slice(
        unparsedEvent.lastIndexOf('") (realdamage') + 16,
        unparsedEvent.lastIndexOf('") (weapon')
      )
    );
  }
  ...
  finalObject.players[playerIDFinder[damageDealerId3]].damageReal +=
    currentReal;
  finalObject.players[playerIDFinder[damageRecieverId3]].damageTaken +=
    damageDealt;
  finalObject.players[playerIDFinder[damageRecieverId3]].damageTakenReal +=
    currentReal;
  ...
  try {
    finalObject.players[playerIDFinder[damageDealerId3]].classStats[
      currentClass
    ].weapons[weaponUsed].damage += damageDealt;
  } catch (error) {
    finalObject.players[playerIDFinder[damageDealerId3]].classStats[
      currentClass
    ].weapons[weaponUsed] = {
  ...
  if (
    finalObject.players[playerIDFinder[damageDealerId3]].damageDivision
      .damageTo[playerIDFinder[damageRecieverId3]] === undefined
  ) {
    finalObject.players[
      playerIDFinder[damageDealerId3]
    ].damageDivision.damageTo[playerIDFinder[damageRecieverId3]] = 0;
    finalObject.players[
      playerIDFinder[damageDealerId3]
    ].damageDivision.damageFrom[playerIDFinder[damageRecieverId3]] = 0;
    finalObject.players[
      playerIDFinder[damageDealerId3]
    ].damageDivision.damageTo[playerIDFinder[damageRecieverId3]] += damageDealt;
  } else {
    finalObject.players[
      playerIDFinder[damageDealerId3]
    ].damageDivision.damageTo[playerIDFinder[damageRecieverId3]] += damageDealt;
  ...
  if (
    finalObject.players[playerIDFinder[damageRecieverId3]].damageDivision
      .damageFrom[playerIDFinder[damageDealerId3]] === undefined
  ) {
    finalObject.players[
      playerIDFinder[damageRecieverId3]
    ].damageDivision.damageTo[playerIDFinder[damageDealerId3]] = 0;
    finalObject.players[
      playerIDFinder[damageRecieverId3]
    ].damageDivision.damageFrom[playerIDFinder[damageDealerId3]] = 0;
    finalObject.players[
      playerIDFinder[damageRecieverId3]
    ].damageDivision.damageFrom[playerIDFinder[damageDealerId3]] += damageDealt;
  } else {
    finalObject.players[
      playerIDFinder[damageRecieverId3]
    ].damageDivision.damageFrom[playerIDFinder[damageDealerId3]] += damageDealt;
  ...
  finalObject.rounds[finalObject.rounds.length - 1].teamScores[
    finalObject.players[playerIDFinder[damageDealerId3]].team
  ].damage += damageDealt;
  ...
  if (
    ...
    finalObject.damagePerInterval[
      finalObject.players[playerIDFinder[damageDealerId3]].team
    ][currentIntervalIndex] += damageDealt;
  }
}
...
function eventDateToSeconds(unparsedEvent) {
  ...
  finalDate =
    ...
    dateArray[1] +
    'T' +
    unparsedEvent.split(' ')[3].slice(0, -1) +
    'Z';
