...
router.get('/match-history/:id&class-played=:classPlayed&map=:map&after=:after&format=:format&order=:order&limit=:limit', (req, response) => {
  ...
  limit ${limit}`)
    ...
    .catch((err) => {
      logger.error('Match history query error', { error: err.message, playerId });
...
router.post('/upload', (req, res) => {
  ...
  // Log upload info
  logger.info('Upload received', { title, map, uploader, hasUpdateLog: !!updatelog });
