...
router.get('/username-search/:username', (req, response) => {
  ...
  LIMIT 5`)
    ...
    .catch((err) => {
      logger.error('Username search query error', { error: err.message, playerUserName });
...
router.get('/peers-search/:id', (req, response) => {
  ...
  pool.query(` select * from peer_table pt where main_id64=${playerId} order by count desc`)
    .then((res) => response.send(res))
    .catch((err) => {
      logger.error('Peers search query error', { error: err.message, playerId });
      response.status(500).json({ error: 'An internal server error occurred' });
    })
});
...
router.get('/enemies-search/:id', (req, response) => {
  ...
  pool.query(` select * from enemy_table pt where main_id64=${playerId} order by count desc`)
    .then((res) => response.send(res))
    .catch((err) => {
      logger.error('Enemies search query error', { error: err.message, playerId });
      response.status(500).json({ error: 'An internal server error occurred' });
    })
});
