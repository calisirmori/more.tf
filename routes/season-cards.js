...
router.get('/leagues', async (req, res) => {
  try {
    const result = await pool.query('SELECT DISTINCT league FROM seasons WHERE league IS NOT NULL ORDER BY league');
...
router.get('/formats/:league', async (req, res) => {
  try {
    ...
    logger.error('Get formats error', { error: err.message, league: req.params.league });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
...
router.get('/seasons/:league/:format', async (req, res) => {
  try {
    ...
    logger.error('Get seasons error', { error: err.message, league: req.params.league, format: req.params.format });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
...
router.get('/design/:league/:format/:seasonid', async (req, res) => {
  try {
    ...
    logger.error('Get card design error', { error: err.message, params: req.params });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
...
router.post('/design', async (req, res) => {
  try {
    ...
    if (!league || !format || !seasonid || !primary_color || !dark_color || !light_color || !accent_color) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    // Validate position values
    if (bg_position_x < 0 || bg_position_x > 100 || bg_position_y < 0 || bg_position_y > 100) {
      return res.status(400).json({ error: 'Background position must be between 0 and 100' });
    }
    ...
    const result = await pool.query(
      `INSERT INTO season_card_designs
       ...
       RETURNING *`,
      [league, format, seasonid, primary_color, dark_color, light_color, accent_color, bg_position_x, bg_position_y]
    );

    res.json({ success: true, design: result.rows[0] });
  } catch (err) {
    logger.error('Save card design error', { error: err.message, body: req.body });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
...
router.get('/bucket-status/:seasonid', async (req, res) => {
  try {
    ...
    logger.error('Get bucket status error', { error: err.message, seasonid: req.params.seasonid });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
...
router.delete('/clear-bucket/:seasonid', async (req, res) => {
  try {
    ...
    if (!listResponse.Contents || listResponse.Contents.length === 0) {
      return res.json({ success: true, deletedCount: 0, message: 'No cards found for this season' });
    }
    ...
    logger.info('Cleared season bucket', { seasonid, deletedCount: objectsToDelete.length });
    ...
    logger.error('Clear bucket error', { error: err.message, seasonid: req.params.seasonid });
    res.status(500).json({ error: 'An internal server error occurred' });
  }
});
